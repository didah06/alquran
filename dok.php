<!DOCTYPE html>
<html>
<head>
  <link href="assets/dist/datatables/dataTables.bootstrap.css" rel="stylesheet" type="text/css" />
  <link href="assets/css/stylesheet.css" rel="stylesheet" type="text/css" />
  <?php include ('inc/head.php'); ?>
  
</head>
<body class="skin-blue">
  <!-- wrapper di bawah footer -->
  <div class="wrapper">

    <?php include ('inc/head2.php'); ?>
    <!-- Left side column. contains the logo and sidebar -->
    <aside class="main-sidebar">
      <!-- sidebar: style can be found in sidebar.less -->
      <?php include ('inc/sidebar.php'); ?>
      <!-- /.sidebar -->
    </aside>


    <!-- Content Wrapper. Contains page content -->
    <div class="content-wrapper">
      <!-- Content Header (Page header) -->
      <section class="content-header">
      <h1>
      <b>DAFTAR SURAT</b>
      </h1>
      <section class="content">
      
      <div class="row">
      <div class="col-md-12">
      <div class="box">

                
            <form action="" method="post">
      
                          <div class="box-body">
                          <div class="col-md-2 col-sm-2 col-xs-2">Surah Alquran
                              <select class="form-control" name="SuraID">
                            <option>Pilih Surah</option>
                            <?php
                            include "koneksi.php";
                       $a="SELECT * FROM daftar_surat";
                       $sql=mysql_query($a);
                       while($data=mysql_fetch_array($sql)){
                       ?>
                       <option value="<?php echo $data['SuraID']?>"><?php echo $data['nama_surat']?></option>
                       <?php
                       }
                       ?>
                          </select></div>     
                        <br>
                        <input type="submit" class="btn btn-primary" value="cari">
                        </div>
                        </form>
                        <div class="row">
                        <button  class="btn btn-primary "><a class="btn btn-primary"  href="preprosesing.php">proses preprocessing dan pembobotan</a></button>
                        
                <div class="col-md-12">
      
                        <div class="box-body">
                        <table id="example1" class="table table-bordered table-striped">
                  <thead>
                  <tr>
                    <th>ID</th>
                    <th>Terjemahan Ayat</th>
                    <th>SuraID</th>
                    <th>No Ayat</th> 
                  </tr>
                  </thead>
                  <tbody>
                    <?php
                    include "koneksi.php";
                   if(isset($_POST['SuraID'])){
                   $sql = "SELECT * from dokumen WHERE SuraID = ".$_POST['SuraID'];
                   $q = mysql_query($sql);
                   while($data = mysql_fetch_array($q)){
                   ?>
                    <tr>
                      <td><?php echo $data['id']; ?></td>
                      <td><?php echo $data['terjemahan_ayat']; ?></td>
                      <td><?php echo $data['SuraID']; ?></td> 
                      <td><?php echo $data['no_ayat']; ?></td> 
                    </tr>
                 <?php 
                  } 
                 } 
                 ?>
                 </tbody>
                 </table>
                 </div>
                 </div>
                 </div>
                 </div>
                 </div>
                 </section>
                 </div>
                 </div>
                 <footer class="main-class">
                 <div class="pull-right hidden-xs">
                 </div>
                 <Strong>Pengelompokkan K-Means</Strong>
                 </footer>
      <?php include ('inc/footer.php'); ?>
      <script src="assets/dist/datatables/jquery.dataTables.js" type="text/javascript"></script>
      <script src="assets/dist/datatables/dataTables.bootstrap.js" type="text/javascript"></script>
      <script type="text/javascript">
        $(function() {
          $("#example1").dataTable();
          $('#example2').dataTable({
            "bPaginate": true,
            "bLengthChange": false,
            "bFilter": true,
            "bSort": true,
            "bInfo": true,
            "bAutoWidth": false
          });
        });
              //waktu flash data :v
        $(function(){
        $('#pesan-flash').delay(4000).fadeOut();
        $('#pesan-error-flash').delay(5000).fadeOut();
        });
      </script>
      </body>
      </html>

    
    